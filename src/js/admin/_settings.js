...
function setupLeaveListener() {
	const listenForChangeForm = document.querySelector('#nep_settings_container');
...
function nep_initAdminColorInputGroup(inputGroupSelector, cssVariable) {
...
function initPreviews() {
	...
	nep_initAdminColorInputGroup(
		"#nep_button_background_color",
		"--pp-button-background-color"
	);

	nep_initAdminColorInputGroup(
		"#nep_button_text_color",
		"--pp-button-text-color"
	);

	const buttonIcons = document.querySelectorAll('input[name=nep_button_options\\[button_icon\\]]');
	...
	const floatButtonIcons = document.querySelectorAll('input[name=nep_button_options\\[floating_button_icon\\]]');
	...
	const text = document.querySelector('#nep_button_text');
	if (text) {
	...
	const buttonEffects = document.querySelectorAll('input[name=nep_button_options\\[button_effect\\]]');
	for (const effect of buttonEffects) {
		effect.addEventListener('change', setEffect);
	}

	function setEffect() {
		const effect = document.querySelector('input[name=nep_button_options\\[button_effect\\]]:checked');
	...
	const disablePeachPayFontCSS = document.querySelector('#nep_disable_default_font_css');
	...
	const paymentIcons = document.querySelector('#nep_payment_method_icons');
	if (paymentIcons) {
...
function updateIcon(icon_value, page) {
	...
	if (icons) {
		...
		switch (icon_value) {
			case 'lock':
				icons.src = nep_wordpress_settings.plugin_asset_url + '/public/img/lock.svg';
				break;
			case 'baseball':
				icons.src = nep_wordpress_settings.plugin_asset_url + '/public/img/baseball-ball-solid.svg';
				break;
			case 'arrow':
				icons.src = nep_wordpress_settings.plugin_asset_url + '/public/img/chevron-circle-right-solid.svg';
				break;
			case 'mountain':
				icons.classList.remove('pp-btn-symbol');
				icons.classList.add('skre-icon');
				icons.src = nep_wordpress_settings.plugin_asset_url + '/public/img/skre.svg';
				break;
			case 'bag':
				icons.src = nep_wordpress_settings.plugin_asset_url + '/public/img/briefcase-solid.svg';
				break;
			case 'shopping_cart':
				icons.src = nep_wordpress_settings.plugin_asset_url + '/public/img/shopping-cart-solid.svg';
				break;
			default:
				if (page === 'shop') {
					icons.src = nep_wordpress_settings.plugin_asset_url + '/public/img/shopping-cart-solid.svg';
...
function setupOCULivePreview() {
	...
	const ocuPrimaryHeader = document.querySelector('#nep_one_click_upsell_primary_header');
	const ocuSecondaryHeader = document.querySelector('#nep_one_click_upsell_secondary_header');
	const ocuAcceptButtonText = document.querySelector('#nep_one_click_upsell_accept_button_text');
	const ocuDeclineButtonText = document.querySelector('#nep_one_click_upsell_decline_button_text');
...
function buttonPageListener() {
	const productEnabler = document.querySelector('#nep_display_on_product_page');
	const productSettings = document.querySelector('#pp-product-page-settings');

	const cartEnabler = document.querySelector('#nep_enabled_on_cart_page');
	const cartSettings = document.querySelector('#pp-cart-page-settings');

	const checkoutEnabler = document.querySelector('#nep_enabled_on_checkout_page');
